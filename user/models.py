...
def get_image_path(instance, filename):
    return os.path.join('user', str(instance.id), filename) #/media/user/<userid>/joke.png

class UserProfile(models.Model): # Piesaistits originalajam user
    user = models.OneToOneField(User)
    description = models.CharField(max_length=300, default='No description provided yet.', blank=True) #blank true lai liktu mieraa
